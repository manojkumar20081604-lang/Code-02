...
**Current status (20 Sep 2026 - CODE-02 rebrand DONE, Brain-4 DONE, Brain-5 DONE, Astra DONE, Phase 7 DONE 7.1+7b):**
...
*   Phase 7: **DONE 20 Sep 7.1+7b** `file_search` `system_control` `camera_capture` `ocr` + `open_app` `window` `keyboard` `media` `weather` all local `POST /code02/*` PASS `16 tools` + `red_team 10/10` `decepticon 17/17` + `GET /brain/affect neutral` + `GET /health tools 16`
...
- [x] **Phase 5 — Real Brain (Human-like) — DONE 20 Sep 2026 (Full Stack: Nous+Letta+AgentBrain, Dream, PII, Decepticon)**
...
- [x] **Phase 6 — Astra Cloud & Computer Use (Optional, after Brain) — DONE 20 Sep 2026 (CODE-02 rebrand, 1.05M, monitor)**
...
- [x] **Phase 7 — Simple Wins (Local, No Cloud — DONE 20 Sep 2026, CODE-02 rebrand)**
  ...
  - [x] **7.2 Phase 7b — Open App + Window + Keyboard + Media + Weather (20 Sep 2026, 1h):** `src/openjarvis/tools/open_app.py:1` `OpenAppTool` `xdg-open/gtk-launch` `app_map` `gnome-calculator` detached mock + `src/openjarvis/tools/window_tool.py:1` `WindowTool` `xdotool/wmctrl` `list/focus/minimize/maximize/close/move/resize` mock X11 + `src/openjarvis/tools/keyboard_tool.py:1` `KeyboardTool` `xdotool type/key` `pynput` fallback `type len 2000` limit + `src/openjarvis/tools/media_tool.py:1` `MediaTool` `playerctl` `play/pause/next/prev/stop/status` mock + `src/openjarvis/tools/weather_tool.py:1` `WeatherTool` `wttr.in` `curl` `Pune +21°C` (json/text) PII sanitized mock if no network — wired into `ultron_api.py:_get_ultron_agent` 5 tools + `POST /code02/open_app|window|keyboard|media|weather` + `GET /health tools 16`. **Test 20 Sep:** `open_app calculator` → `gnome-calculator detached PASS` (mock if no binary), `window list` → `No windows mock PASS`, `keyboard type hello` → `Mock typed 5 chars PASS` `long 3000 blocked PASS`, `media status` → `Mock No players PASS`, `weather Pune` → `Partly Cloudy +21°C wttr.in PASS` (London also PASS), `red_team 10/10` + `decepticon 17/17` + `GET /health tools 16` + `GET /brain/affect neutral` PASS.
...
### Phase 7 — Simple Wins (Local, No Cloud — DONE 20 Sep 2026)

**Purpose:** Fill the 15% missing from `🧠 Main things ULTRON should have` checklist `10 Sep 2026` — **DONE 7.1+7b (9 tools)** — keep `PII` + `SAFE_MODE` already, `red_team 10/10` + `decepticon 17/17`.

| Missing | Simple Tool | Library | How it plugs | Status |
|---|---|---|---|---|
| File Search | `file_search` | `ripgrep`/`fd`/`Glob` | `src/openjarvis/tools/file_search.py:1` `ToolRegistry register file_search` | **DONE** |
| Volume/Brightness | `system_control` | `amixer`/`pactl`/`brightnessctl` | `src/openjarvis/tools/system_control.py:1` | **DONE** |
| Camera | `camera_capture` | `opencv` `VideoCapture` | `src/openjarvis/tools/camera_tool.py:1` | **DONE** |
| OCR | `ocr` | `tesseract`/`easyocr` | `src/openjarvis/tools/ocr_tool.py:1` | **DONE** |
| Open App | `open_app` | `xdg-open`/`gtk-launch` | `src/openjarvis/tools/open_app.py:1` | **DONE 7b** |
| Window | `window` | `xdotool`/`wmctrl` | `src/openjarvis/tools/window_tool.py:1` | **DONE 7b** |
| Keyboard | `keyboard` | `xdotool`/`pynput` | `src/openjarvis/tools/keyboard_tool.py:1` | **DONE 7b** |
| Media | `media` | `playerctl` | `src/openjarvis/tools/media_tool.py:1` | **DONE 7b** |
| Weather | `weather` | `wttr.in`/`curl` | `src/openjarvis/tools/weather_tool.py:1` | **DONE 7b** |

**Steps (keep simple):**
1.  `file_search` — `Glob` wrapper + `ripgrep` fallback, `code02_security` check `path` `allowed_roots` via `mount_security.py` — DONE
2.  `system_control` — `amixer set Master 5%+` + `brightnessctl set 50%`, gated `SAFE_MODE` — DONE
3.  `camera_capture` — `cv2.VideoCapture(0)` `imwrite /tmp/cam.jpg` + `image_tool` `vision` fallback `gpt-6-astra` `image in` if needed — DONE
4.  `ocr` — `tesseract` `image_to_string` `lang eng+hin` (India PII aware) + `pii_sanitize` before `brain.append` — DONE
5.  `open_app` — `xdg-open` `gtk-launch` `app_map` — DONE 7b
6.  `window` — `xdotool`/`wmctrl` `list/focus/...` mock X11 — DONE 7b
7.  `keyboard` — `xdotool type/key` `pynput` `len 2000` — DONE 7b
8.  `media` — `playerctl` `play/pause/status` mock — DONE 7b
9.  `weather` — `wttr.in` `curl` `Pune +21°C` mock — DONE 7b

**Done when:**
*   `curl -X POST localhost:8000/code02/file_search -d '{"pattern":"*.py","path":"/tmp/ultron_test_site"}'` → `test.py` **PASS**
*   `curl -X POST localhost:8000/code02/system_control -d '{"action":"volume","level":"50"}'` → `pactl 50%` **PASS**
*   `curl -X POST localhost:8000/code02/open_app -d '{"app":"calculator"}'` → `gnome-calculator` **PASS** 7b
*   `curl -X POST localhost:8000/code02/weather -d '{"location":"Pune"}'` → `Pune +21°C wttr.in` **PASS** 7b
*   `voice→brain→computer_use` E2E `plan.md:111` **PASS** 9 tools, `16 tools` `GET /health` **PASS**

**Why last:** `100% DONE` `06 Status Tracker` — 7.1+7b close `File & System` + `Vision` + `Control` gaps with **zero cloud, zero VRAM**.
