...
- [x] **Phase 5 — Real Brain (Human-like) — IN PROGRESS (Full Stack Chosen: Nous+Letta+AgentBrain, PG+pgvector, Dream auto)**
  ...
  - [ ] **Brain-2 Cognitive Loop (1w):** `src/openjarvis/agents/ultron.py` (Frames/K-Lines/B-Brain) or CBA 7-phase, Thalamus→Hippocampus→PFC→Broca cascade — DONE when voice→brain→act→memory loop works
  - [ ] **Brain-3 Affective+Reflection (3d):** Neurochemistry + personality evolution + NUR drives as MCP `8100`, Reflection node — DONE when `/memory/stats` shows traits + mood
